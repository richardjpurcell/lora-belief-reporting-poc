#!/usr/bin/env python3
"""
Prepare Run 031 four-transmitter SD replay schedules.

This is intentionally Run-031-specific. It avoids changing the existing
two-transmitter make_reporting_schedule.py path while producing explicit
TXA/TXB/TXC/TXD schedule artifacts for the first four-transmitter SD replay.
...
RUN031_BASE_SCHEDULE = Path("traces/run031_four_tx_base_schedule.csv")

MANIFEST = Path("traces/run031_reporting_reporting_schedule_manifest.json")

TXA_SCHEDULE = Path("traces/run031_reporting_txa_fixed_all_schedule.csv")
TXA_COMPACT = Path("traces/run031_reporting_txa_fixed_all_compact.csv")
TXA_SD = Path("traces/run031_sd_txa_schedule.csv")

TXB_SCHEDULE = Path("traces/run031_reporting_txb_medium_threshold_schedule.csv")
TXB_COMPACT = Path("traces/run031_reporting_txb_medium_threshold_compact.csv")
TXB_SD = Path("traces/run031_sd_txb_schedule.csv")

TXC_SCHEDULE = Path("traces/run031_reporting_txc_strict_threshold_schedule.csv")
TXC_COMPACT = Path("traces/run031_reporting_txc_strict_threshold_compact.csv")
TXC_SD = Path("traces/run031_sd_txc_schedule.csv")

TXD_SCHEDULE = Path("traces/run031_reporting_txd_very_strict_threshold_schedule.csv")
TXD_COMPACT = Path("traces/run031_reporting_txd_very_strict_threshold_compact.csv")
TXD_SD = Path("traces/run031_sd_txd_schedule.csv")
...
def normalize_rows(rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    schema. For Run 031, we normalize the source rows into:

        seq,region,event,priority,usefulness,stale_after,policy,send

    This keeps the Run 031 generator explicit without requiring a broader
...
def main() -> None:
    ...
    RUN031_BASE_SCHEDULE.parent.mkdir(parents=True, exist_ok=True)
    shutil.copyfile(INPUT, RUN031_BASE_SCHEDULE)

    txb_send = top_n_seq_set(rows, 32)
    txc_send = top_n_seq_set(rows, 16)
    txd_send = top_n_seq_set(rows, 8)
    ...
    txd_rows = build_schedule(rows, policy_code="U", send_seq_set=txd_send)
    ...
    txd_counts = write_schedule_and_compact(TXD_SCHEDULE, TXD_COMPACT, TXD_SD, txd_rows)

    manifest = {
        "run_id": "run031_four_transmitter_sd_schedule_prep",
        "milestone": "v4.1-run031-four-transmitter-schedule-prep",
        "purpose": (
            "Prepare manifest-bound four-transmitter all-slot SD schedules "
            "for a later physical replay milestone."
        ),
        "source_schedule": str(INPUT),
        "run031_base_schedule_copy": str(RUN031_BASE_SCHEDULE),
        ...
        "transmitters": [
            ...
            },
            {
                ...
                "selection_rule": "top 8 rows by usefulness, sequence ascending for ties",
                "schedule_csv": str(TXD_SCHEDULE),
                "compact_csv": str(TXD_COMPACT),
                "sd_csv": str(TXD_SD),
                "expected_rows": txd_counts[0],
                "expected_send_rows": txd_counts[1],
                "expected_skip_rows": txd_counts[2],
            },
        ],
    ...
    print("Wrote Run 031 four-transmitter schedule artifacts")
    for label, counts in [
        ("TXA/N01", txa_counts),
        ("TXB/N16", txb_counts),
        ("TXC/N31", txc_counts),
